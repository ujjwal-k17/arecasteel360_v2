...
export default function WoodenPalletsTab() {
  ...
  const { data: consumptions } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('pallet_consumptions').select('*').order('consumption_date', { ascending: false });
      if (error) throw error;
      return data as PalletConsumption[];
    },
  });
  ...
  const insertSKU = useMutation({
    mutationFn: async (size: string) => {
      const { error } = await supabase.from('pallet_skus').insert({ pallet_size: size });
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['pallet_skus'] }),
  });

  const insertPurchase = useMutation({
    mutationFn: async (data: Omit<PalletPurchase, 'id' | 'created_at'>) => {
      const { error } = await supabase.from('pallet_purchases').insert(data);
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['pallet_purchases'] }),
  });

  const insertConsumption = useMutation({
    mutationFn: async (data: Omit<PalletConsumption, 'id' | 'created_at'>) => {
      const { error } = await supabase.from('pallet_consumptions').insert(data);
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['pallet_consumptions'] }),
  });

  // Calculate balances per SKU
  ...
  const handlePurchaseSubmit = async () => {
    ...
    try {
      ...
      setPurchaseForm({ date: '', supplier: '', weight: '', pcs: '', rate: '' });
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Expected columns: Pallet Size, Weight (Kg), # of Pcs
      for (const row of rows) {
        ...
        if (!sku) {
          const { data: newSku, error: skuErr } = await supabase.from('pallet_skus').insert({ pallet_size: size }).select().single();
          if (skuErr) throw skuErr;
          sku = newSku as PalletSKU;
        }

        // Add as opening inventory (purchase with today's date)
        if (weight > 0 || pcs > 0) {
          await supabase.from('pallet_purchases').insert({
  ...
  const handleDownloadTemplate = () => {
    ...
    XLSX.writeFile(wb, 'pallet_inventory_template.xlsx');
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={showAddSKU} onOpenChange={setShowAddSKU}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddSKU} disabled={insertSKU.isPending}>
              {insertSKU.isPending ? 'Adding...' : 'Add SKU'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!showPurchase} onOpenChange={() => setShowPurchase(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Record Purchase - {showPurchase?.pallet_size}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
            ...
            <div>
              ...
              <Input value={purchaseForm.supplier} onChange={e => setPurchaseForm(v => ({ ...v, supplier: e.target.value }))} />
            </div>
            <div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handlePurchaseSubmit} disabled={insertPurchase.isPending}>
              {insertPurchase.isPending ? 'Saving...' : 'Record Purchase'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
/* ── Purchase Summary section: date-wise, vendor + sizes dropdown ── */
function PurchaseSummarySection({ purchases, skus }: { purchases: PalletPurchase[]; skus: PalletSKU[] }) {
  ...
  const grouped = useMemo(() => {
    const m = new Map<string, { date: string; supplier: string; totalWeight: number; totalPcs: number; sizes: { size: string; weight: number; pcs: number }[] }>();
    purchases.forEach(p => {
      const key = `${p.purchase_date}|${p.supplier || '-'}`;
      if (!m.has(key)) m.set(key, { date: p.purchase_date, supplier: p.supplier || '-', totalWeight: 0, totalPcs: 0, sizes: [] });
