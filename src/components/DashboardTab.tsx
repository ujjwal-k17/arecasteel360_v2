...
// ===== Tally Sync =====
type TallyStockItem = { name: string; quantity: string; value: string };

function TallySyncSection() {
  const [items, setItems] = useState<TallyStockItem[] | null>(null);
  ...
  return (
    <section className="rounded-xl border bg-card p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold">Tally Sync</h3>
          <p className="text-xs text-muted-foreground">
            {fetchedAt
              ? `Last synced: ${new Date(fetchedAt).toLocaleString('en-IN')}`
              : 'Pull live stock items from Tally Server'}
          </p>
        </div>
        <Button onClick={handleSync} disabled={loading} size="sm" className="gap-2">
          <RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin' : ''}`} />
          {loading ? 'Syncing…' : 'Sync from Tally'}
        </Button>
      </div>
              ...
              {items.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} className="text-center text-xs text-muted-foreground py-6">
                    No stock items returned by Tally.
                  </TableCell>
                </TableRow>
              ) : (
                items.map((it, i) => (
                  <TableRow key={`${it.name}-${i}`}>
                    ...
                    <TableCell className="text-xs font-mono-num text-right">{it.quantity || '—'}</TableCell>
                    <TableCell className="text-xs font-mono-num text-right">{it.value || '—'}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
